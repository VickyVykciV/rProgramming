...
We see, there is not much of gradiation in the predicted probabilities of each quantile, so we have to scale the data
...
```{r, echo=F}
Pred_Nm = data.frame(Target = Dev_Sample$Attrition_Num,  
...
with(Pred_Nm,table(Target,Pred_Class))
mis_class_error<-(150/2058)*100
...
compute.output = compute(nnModel_2, Hold_Out_Sample_2) 
Hold_Out_Sample_2$Predict.score = compute.output$net.result 
View(Hold_Out_Sample_2)  
